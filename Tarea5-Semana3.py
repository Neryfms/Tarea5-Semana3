def menu():
    print("Bienvenido")
    print("Este es un  pequeño programa que le ayudará a calcular el area de un circulo")
    print("¿Qué desea hacer?")
    print("1. Calcular el area de un circulo")
    print("2. Salir")
    print("Elija una de las dos opciones ingresando el numero correspondiente")
    op = input ("Escriba la opcion:")
    op = int(op)
    if op==1: Area_de_circulo()
    else: exit()

def Area_de_circulo():
    import math
    print("")
    print("Para calcular el area de un circulo, necesitamos conocer su radio.")
    print("")
    print("Por favor, Ingrese el valor del radio del circulo:")
    r = float (input())
    print("¡Bien hecho!")
    print("")
    print("Estamos realizando la operación")
    print("")
    print("Por favor, espere...")
    print("")
    a= math.pi * (r*r)
    print("El area del circulo de ",r," es: ",a)
    print("")
    menu()
menu()